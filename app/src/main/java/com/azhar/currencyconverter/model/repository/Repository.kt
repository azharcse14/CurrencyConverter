...
class Repository(private val converterDatabase: ConverterDatabase,   private val converterDao: ConverterDao) {
    ...
    fun getCurrencyList(): Flow<CurrencyList> = flow {
        ...
        for (key in list.currencies.keySet()){
            ...
            converterDatabase.converterDao().addCurrencies(Currency(key, list.currencies[key].asString))
        }
    ...
    fun getQuotes(): Flow<QuoetsList> = flow {
        ...
        for (key in data.quotes.keySet()){
            converterDatabase.converterDao().addQuotes(Quote(key.substring(3),  data.quotes[key].asString))
        }
